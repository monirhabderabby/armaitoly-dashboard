...
/**
 * Used for trimming non-alphanumeric characters from the start/end.
 */
...
export const TagsInput = React.forwardRef<HTMLDivElement, TagsInputProps>(
  (
    ...
    const addValue = React.useCallback(
      (val: string) => {
        const parsed = val.replace(FORMATTING_REGEX, "").trim();

        if (
          parsed.length > 0 &&
          !value.includes(parsed) &&
    ...
    return (
      ...
      >
        <div
          ...
          dir={dir}
          className={cn(
            "flex items-center flex-wrap gap-1 overflow-hidden rounded-lg bg-background p-1 ring-1 ring-muted",
            {
        ...
        >
          {value.map((item, index) => (
            <Badge
              ...
              className={cn(
                "relative flex items-center gap-1 truncate rounded px-1 data-[active='true']:ring-2 data-[active='true']:ring-muted-foreground aria-disabled:cursor-not-allowed aria-disabled:opacity-50",
              )}
              variant="secondary"
            >
